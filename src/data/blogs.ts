...
export const sortPostsByDateDesc = (posts: BlogPostMeta[]) =>
  [...posts].sort(
    (a, b) => parseBlogDate(b.date).getTime() - parseBlogDate(a.date).getTime(),
  );

export const sortBlogsByRecentPostDesc = (blogList: BlogMeta[]) =>
  [...blogList]
    .map((blog) => ({
      ...blog,
      posts: sortPostsByDateDesc(blog.posts),
    }))
    .sort((a, b) => {
      ...
      if (!latestA || !latestB) {
        return Number(Boolean(latestB)) - Number(Boolean(latestA));
      }

      return (
        parseBlogDate(latestB.date).getTime() - parseBlogDate(latestA.date).getTime()
      );
    });
  ...
  },
  {
    ...
    posts: [
      ...
      },
      {
        ...
        date: "15/11/2024",
        excerpt: "Comment integration-unify est structuré : deux pipelines Python, une chaîne critique et des modèles domaine exigeants.",
      },
      {
        slug: "pipeline-etl",
        title: "Concevoir un pipeline ETL pour l'industrie",
        date: "10/01/2025",
        excerpt: "Plongée dans PythonTemplateExtraction : lecture multi-source Excel et SQL Server, transformation et écriture vers PlanetTogether.",
      },
      {
        slug: "qualite-logicielle",
        title: "Qualité logicielle en milieu industriel : tests, typage et débogage",
        date: "01/03/2025",
        excerpt: "Audit Pyright (323 → 0 erreurs), renforcement de la suite pytest et correction d'un bug critique Machine unhashable en production.",
      },
    ],
  },
  {
...
export default sortBlogsByRecentPostDesc(blogs);
